...
const [migrationRows]=await db.query('SELECT filename,checksum_sha256 FROM schema_migrations ORDER BY filename');if(migrationRows.length<9)throw new Error(`SCHEMA_MIGRATION_COUNT:${migrationRows.length}`);
const [supplierRows]=await db.query('SELECT COUNT(*) AS n FROM suppliers');const [categoryRows]=await db.query('SELECT COUNT(*) AS n FROM categories');const [attestationRows]=await db.query('SELECT COUNT(*) AS n FROM launch_attestations');if(Number(supplierRows[0].n)<5)throw new Error('SCHEMA_SUPPLIER_SEED_MISSING');if(Number(categoryRows[0].n)<8)throw new Error('SCHEMA_CATEGORY_SEED_MISSING');if(Number(attestationRows[0].n)<4)throw new Error('SCHEMA_LAUNCH_ATTESTATIONS_MISSING');
console.log(JSON.stringify({ok:true,tables:requiredTables.length,migrations:migrationRows.length,suppliers:Number(supplierRows[0].n),categories:Number(categoryRows[0].n),launch_attestations:Number(attestationRows[0].n)}));
await db.end();
