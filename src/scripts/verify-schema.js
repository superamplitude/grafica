...
const [migrationRows]=await db.query('SELECT filename,checksum_sha256 FROM schema_migrations ORDER BY filename');if(migrationRows.length<12)throw new Error(`SCHEMA_MIGRATION_COUNT:${migrationRows.length}`);
