...
import { parseHtmlXlsPriceTable } from '../domain/supplier-price-parser.js';
...
try{
  ...
  const [existing]=await db.execute('SELECT id,status,row_count,category_count FROM supplier_price_imports WHERE source_checksum_sha256=? LIMIT 1',[parsed.checksum_sha256]);
  if(existing[0]){
    console.log(JSON.stringify({ok:true,idempotent:true,import_id:Number(existing[0].id),checksum_sha256:parsed.checksum_sha256,row_count:Number(existing[0].row_count),category_count:Number(existing[0].category_count),status:existing[0].status},null,2));
    await db.end();
    process.exit(0);
  }
  ...
  try{
    ...
    await conn.execute(`UPDATE supplier_price_rows r
      JOIN product_variants v ON v.external_code=r.source_code
      SET r.matched_variant_id=v.id,r.match_status='exact'
    ...
    await conn.execute('UPDATE supplier_price_imports SET row_count=?,category_count=?,matched_count=?,conflict_count=? WHERE id=?',[
      Number(stats.total||0),Number(stats.categories||0),Number(stats.matched||0),Number(stats.conflicts||0),importId
    ]);
    await conn.commit();
    console.log(JSON.stringify({ok:true,idempotent:false,import_id:importId,checksum_sha256:parsed.checksum_sha256,source_date:parsed.source_date,row_count:parsed.row_count,category_count:parsed.category_count,matched_count:Number(stats.matched||0),status:'staged',automatic_apply:false},null,2));
