...
`, [
  ...
  `Produto ${label}`,
  `produto-${id}`,
  `Produto de integração ${label}`,
  `Descrição comercial de integração suficientemente longa para validar o catálogo público sem depender de conteúdo externo ${label}.`,
  49.90
]);
...
await db.execute(`
  ...
  VALUES (?,?,?,?,?,0,0,0,?,0,1000,'9x5 cm','4x4',3,'available','active')
`, [productId,`CIV-${id}`,`EXT-${id}`,`1000 un. ${label}`,49.90,49.90]);

async function insertMedia(kind, objectKey, originalName, mimeType) {
  const [result] = await db.execute(`
    INSERT INTO media_objects (kind,visibility,storage_provider,bucket_name,object_key,original_name,mime_type,size_bytes,metadata_json)
    VALUES (?,'public','cloudflare-r2','grafica',?,?,?,100,JSON_OBJECT('ci',true))
  `,[kind,objectKey,originalName,mimeType]);
...
await db.execute(`
  INSERT INTO media_asset_reviews
    (media_id,usage_type,photo_type,supplier_branding,price_text,license_status,review_status,reviewed_at)
  VALUES (?,'product','real','clear','clear','licensed','approved',NOW())
`,[photoId]);
...
await db.execute(`
  INSERT INTO media_asset_reviews
    (media_id,usage_type,photo_type,supplier_branding,price_text,license_status,review_status,reviewed_at)
  VALUES (?,'hero','real','clear','clear','owned','approved',NOW())
`,[bannerMediaId]);
...
await db.execute(`
  INSERT INTO product_templates
    (product_id,media_id,template_type,version_label,side,width_mm,height_mm,bleed_mm,brand_neutral,verified_at,status)
  VALUES (?,?,'pdf','PDF produção','general',90,50,2,1,NOW(),'active')
`,[productId,templateMediaId]);
await db.execute(`
  INSERT INTO product_templates
    (product_id,media_id,template_type,version_label,side,width_mm,height_mm,bleed_mm,brand_neutral,verified_at,status)
  VALUES (?,?,'pdf','PDF não verificado','general',90,50,2,0,NULL,'active')
`,[productId,templateMediaId]);
