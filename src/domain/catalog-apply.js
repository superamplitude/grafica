...
import { commercialPrices, reviewPricingInput } from './pricing-policy.js';
...
function buildProductCopy(group){
  ...
  return {short_description:short.slice(0,1400),description:description.slice(0,6000),metadata:{variant_count:variants,sizes,print_configurations:prints,quantity_min:qty.min,quantity_max:qty.max,production_days_min:days.min,production_days_max:days.max}};
}
...
export async function applySupplierPriceImport(conn,{importId,actorId,confirmApply=false}){
  ...
  let products=0,variants=0,publicChanges=0,resellerChanges=0,reviewVariants=0;
  for(const group of groups){
    const categoryId=categoryIds.get(group.category);const featured=FEATURED.has(group.category)?1:0;const copy=buildProductCopy(group);
    ...
    for(const row of group.rows){
      ...
      const pricingReview=reviewPricingInput({supplierCost,quantity:Number(qty||1)});
      const {publicResult:pub,resellerResult:res}=commercialPrices({supplierCost,additionalCost:0});
      const availability=pricingReview.status==='published'?'available':'unavailable';
      if(pricingReview.status==='published')minPrice=minPrice===null?pub.price:Math.min(minPrice,pub.price);else reviewVariants++;
      ...
      await conn.execute(`INSERT INTO product_variants (product_id,sku,external_code,name,price,cost,supplier_cost,additional_cost,public_price,reseller_price,quantity,size_label,print_configuration,production_days,availability,pricing_review_status,pricing_review_reason,attributes_json,production_json,status)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'active')
        ON DUPLICATE KEY UPDATE product_id=VALUES(product_id),sku=VALUES(sku),name=VALUES(name),price=VALUES(price),cost=VALUES(cost),supplier_cost=VALUES(supplier_cost),additional_cost=0,public_price=VALUES(public_price),reseller_price=VALUES(reseller_price),quantity=VALUES(quantity),size_label=VALUES(size_label),print_configuration=VALUES(print_configuration),production_days=VALUES(production_days),availability=VALUES(availability),pricing_review_status=VALUES(pricing_review_status),pricing_review_reason=VALUES(pricing_review_reason),attributes_json=VALUES(attributes_json),production_json=VALUES(production_json),status='active'`,
        [productId,`CP-${code}`.slice(0,120),code,`${Number(qty||1)} un. · ${size||'formato'} · ${String(colors||'').toUpperCase()} · ${Number(days||0)} dia(s)`.slice(0,255),pub.price,supplierCost,supplierCost,0,pub.price,res.price,Math.max(1,Number(qty||1)),size||null,colors||null,Number(days)||null,availability,pricingReview.status,pricingReview.reason,JSON.stringify({source_description:description,source_weight:weight,source_category:group.category}),JSON.stringify({supplier_id:Number(imp.supplier_id),source_code:code,import_id:importId})]);
      ...
      await conn.execute(`UPDATE supplier_price_rows SET matched_variant_id=?,match_status='exact',review_status=? WHERE import_id=? AND source_code=?`,[after.id,pricingReview.status==='published'?'approved':'pending',importId,code]);
  ...
  await conn.execute(`UPDATE supplier_price_imports SET status='applied',matched_count=?,conflict_count=?,reviewed_by_user_id=?,reviewed_at=COALESCE(reviewed_at,NOW()),applied_at=NOW() WHERE id=?`,[variants,reviewVariants,actorId,importId]);
  return {ok:true,idempotent:false,import_id:importId,products,variants,categories:categories.length,public_price_changes:publicChanges,reseller_price_changes:resellerChanges,pricing_review_variants:reviewVariants};
}
