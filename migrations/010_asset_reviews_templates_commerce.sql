...
SET @ddl := IF((SELECT COUNT(*) FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA=@db AND TABLE_NAME='product_templates' AND COLUMN_NAME='verified_by_user_id' AND REFERENCED_TABLE_NAME='staff_users')=0,'ALTER TABLE product_templates ADD CONSTRAINT fk_template_verified_user FOREIGN KEY (verified_by_user_id) REFERENCES staff_users(id) ON DELETE SET NULL','SELECT 1'); PREPARE s FROM @ddl; EXECUTE s; DEALLOCATE PREPARE s;
